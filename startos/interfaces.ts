...
export const setInterfaces = sdk.setupInterfaces(async ({ effects }) => {
  ...
  // secure: null still allocates a plaintext external port. It is reachable over the LXC bridge,
  // which is the address dependents resolve, and from nowhere else, so off the box the TLS one is
  // all there is.
  ...
  const main = sdk.createInterface(effects, {
    ...
    type: 'api',
    masked: false,
    // protocol: null leaves the origin scheme-less, which renders every address as a bare
    // host:port with nothing marking it as TLS.
